...
* Deprecated 更强调“逐步淘汰，有替代方案，但仍然保留使用的可能性”。
* Obsolete 更强调“彻底过时，几乎不可用，应立刻停止使用”。

## 在C++中，什么是pure function

在C++中，**pure function** 是一种函数设计概念，指的是**纯函数**。纯函数具有以下两个主要特性：

1. **引用透明性（Referential Transparency）**  
   - 函数的返回值仅取决于其输入参数，而不依赖于任何外部状态或可变数据。  
   - 给定相同的输入参数，纯函数始终返回相同的输出。

2. **无副作用（No Side Effects）**  
   - 函数在执行时不会修改任何外部状态（例如全局变量、类成员变量或 I/O 操作）。  
   - 它不会对程序的其他部分产生可观察的影响。
...
```cpp
int add(int a, int b) {
    return a + b; // 仅依赖参数，且没有副作用
}
```

### 非纯函数的示例：

下面的函数不满足纯函数的要求，因为它依赖或修改了外部状态：
...
```cpp
int globalMultiplier = 2;

int multiply(int a) {
    return a * globalMultiplier; // 返回值依赖外部变量
}
```
...
void printMessage(const std::string& message) {
    std::cout << message; // 执行了 I/O 操作
}
```

### 纯函数的优点：

* **可预测性**：给定相同的输入，纯函数的输出始终相同。
* **易测试性**：由于纯函数不依赖外部状态，可以独立于上下文进行测试。
* **易并行化**：纯函数可以安全地在多线程环境中调用，无需担心数据竞争。
* **函数式编程友好**：纯函数是函数式编程的核心概念。

在 C++ 中虽然没有语言级别的强制机制来标记纯函数，但可以通过设计习惯和工具（如 `const` 修饰符）来尽量保证函数的纯度。
