...
* 移动构造函数在创建对象时触发，用于“搬走”一个临时对象或不再使用的对象的资源。
* 移动赋值函数在赋值操作中触发，用于“搬走”一个对象的资源到另一个已经存在的对象中。
...
1. 移动语义（Move Semantics）：
   ...
   这是通过C++11中引入的移动构造函数和移动赋值运算符来实现的。它们使用右值引用作为参数，从而有效地“移动”资源，而不是复制它们。
...
转发引用（Forwarding Reference）是C++11中引入的一种引用类型，用于实现完美转发（Perfect Forwarding）。转发引用与传统的左值引用和右值引用不同，它具有非常灵活的特性。
...
在这种情况下， `const` 修饰返回值是没有意义的。函数返回的值是一个临时对象，无法直接修改，因此即使没有 `const` 限定，它也是不可修改的。加上 `const` 并不能增加额外的限制或影响代码的行为。
...
在这种情况下， `const` 是有意义的。 `const` 修饰符意味着调用该函数时，返回的引用对象不能被修改。如果函数返回的是一个对象的引用，并且不希望调用者修改它，那么 `const` 就是必须的。
...
* 如果返回类型是值类型，如 `int`，`const` 前缀没有意义，因为返回的临时对象本身就是不可修改的。
* 如果返回类型是引用或指针类型，则 `const` 可以保护返回的对象，防止调用者修改它。在这种情况下，`const` 是有意义的。
...
“Use `using namespace` directives for transition” 强调的是在代码库过渡期间的一个临时性策略，帮助开发者逐步从一个命名空间或设计转移到另一个，而不是破坏性地一次性修改整个代码库。在完成过渡后，应该回归到更规范的命名空间使用方式，即显式地指定命名空间，而不是依赖于 `using namespace` 。
...
这段 C++ 代码不会导致栈溢出，而是可能会吃满内存，原因在于：动态内存分配的机制使得大量的内存分配是在堆上而不是栈上进行的。
...
1. `std::vector<int> largeVector(1'000'000)`：这是一个局部变量 `largeVector`，它的大小是 1, 000, 000 个 `int` 元素。
   - `std::vector` 是一个动态数组类，它会在堆上为其内容（即这 1, 000, 000 个 `int` 元素）分配空间。尽管 `largeVector` 这个对象本身是在栈上分配的（它只占用很少的栈空间，因为它存储的仅仅是指向堆上数据的指针、大小等元数据），但它所管理的 1, 000, 000 个 `int` 元素实际上是在堆上分配的。
...
当你在函数中声明一个 `std::vector` 变量时， `vector` 本身是一个对象，这个对象包含了用于管理动态数组的内部指针和一些控制信息。这些信息会存储在栈空间上。通常，这些数据包括：
...
`std::vector` 使用动态数组来存储它的元素，而这些元素是通过动态内存分配器（通常是 `std::allocator` ）分配的，这部分内存存储在堆空间上。具体来说：
...
* 非静态全局变量：具有全局作用域，可以被其他文件中的代码访问（前提是通过 `extern` 声明）。
* 静态全局变量：具有文件作用域，只能在声明它的文件中访问。无法通过 `extern` 在其他文件中引用。

### 2. 链接属性  

* 非静态全局变量：具有外部链接性（external linkage），在多个文件中声明的同名变量会被视为同一个变量。
* 静态全局变量：具有内部链接性（internal linkage），即使多个文件中定义了同名的静态全局变量，它们也是彼此独立的。
...
1. 非静态全局变量在 `file1.cpp` 中定义，在 `file2.cpp` 中通过 `extern` 声明并使用。
2. 静态全局变量仅在 `file1.cpp` 中有效，`file2.cpp` 无法访问。
...
在代码接口中，deprecated（不推荐使用）和obsolete（过时的）有一些细微的区别：
...
2. Obsolete（过时的）：
   ...
   - 有时，obsolete 可以等同于deprecated，但通常意味着更强烈的弃用警告，甚至可能没有任何替代方案。
...
在 C++20 中，编译通过而在 C++17 中编译失败，主要是因为 C++20 引入了返回值类型折叠（RVO，Return Value Optimization）和临时对象的标准化转移（NRVO，Named Return Value Optimization）等特性。
...
* 严格定义下的副作用：  
  若纯函数要求完全无副作用（包括内部状态修改），则记忆化函数可能被排除在外。但大多数实际场景中，只要副作用不影响可观察行为，仍可接受为纯函数。
...
1. 未声明移动操作的情况  
   ...
   - 编译器不会自动生成默认的移动构造函数和移动赋值运算符（因为用户显式声明了拷贝操作）。
...
若显式定义派生类的拷贝或移动操作，并显式调用基类的合法构造函数/赋值运算符（如默认构造），则派生类的操作可以生效。但需注意这可能引发逻辑错误（基类部分未正确拷贝）。
...
在C++中，使用 `using` 别名（类型别名）来更改接口函数的参数类型不会导致ABI（应用程序二进制接口）变更，但需满足以下条件：
...
当派生类的构造函数未显式调用基类构造函数时，编译器会自动尝试调用基类的默认构造函数（无参构造函数）。例如：
...
如果基类没有默认构造函数（例如，只有带参数的构造函数），则派生类必须显式调用基类的某个构造函数，否则会编译报错：
...
即使基类存在默认构造函数，若其访问权限为 `private` 且派生类无权访问，隐式调用仍会失败：
...
虽然操作系统会在程序结束时回收内存，但依赖此行为是不良实践。始终应通过 `delete` 或智能指针主动释放内存，以确保资源管理的正确性和代码的健壮性。
...
在C++中， `std::forward` 是一个与完美转发（Perfect Forwarding）相关的工具，用于在模板编程中保留参数的值类别（左值或右值），确保参数在传递过程中保持原始类型。以下是其核心要点：
...
* 典型场景：与通用引用（`T&&`，即转发引用）配合使用，将参数无损地传递给其他函数。
...
* 如果成员函数可能抛出异常，需确保异常发生时不变式仍然成立（即提供“强异常安全保证”）。
...
在 C++ 中， `using namespace` 指令的作用域是局部的，离开其所在的局部作用域后，该指令不再生效。具体规则如下：
...
在面向对象编程中，基类（Base Class）、父类（Parent Class）和超类（Superclass）这三个术语通常指同一个概念，即被其他类继承的类。但它们可能在不同编程语言或上下文中存在细微差异：
...
在C++中，聚合类（Aggregate Class） 是一种特殊类型的类或结构体，允许通过花括号初始化列表（brace-enclosed list） 直接初始化其成员。这种特性使得聚合类在表示简单数据结构时非常高效。以下是聚合类的核心要点：
...
为了解决这个问题，我们需要使用一个容差 (tolerance) 来判断两个浮点数是否“足够接近”。如果两个数的绝对差小于这个容差，我们就认为它们是相等的。

`epsilon` 提供了一个衡量浮点数精度（特别是相对精度）的基准，可以用来设定这个容差。
...
`std::numeric_limits<T>::epsilon()` 本身表示 1.0 附近的浮点精度，它的主要作用是作为衡量浮点类型精度的参考值，并被广泛用于设计或实现浮点数比较函数，以判断两个浮点数是否在允许的误差范围内“相等”，而不是依赖于不精确的直接相等比较 ( `==` )。
...
在C++中， `friend` （友元）关键字用于授予一个非成员函数、一个类，或者 一个类的成员函数 对另一个类的 `private` 和 `protected` 成员的访问权限。
...
1.  操作符重载 (Operator Overloading): 这是最常见的用法之一。例如，当你重载 `<<` (输出) 或 `>>` (输入) 操作符，或者重载二元操作符，而左操作数不是类对象时，你需要这个操作符函数能够访问类对象的私有数据。将操作符函数声明为类的友元，就可以实现这一点。
...
`friend` 声明必须出现在类定义的内部。它可以出现在 `public` , `protected` , `private` 中的任何一个区域，其放置的位置不会影响友元声明的访问权限。
...
将整个类声明为另一个类的友元。友元类的所有成员函数都可以访问被声明为友元类的类的 `private` 和 `protected` 成员。
...
只将一个类的特定成员函数声明为另一个类的友元。这比将整个类声明为友元提供了更细粒度的控制。
...
*   声明类的成员函数为友元时，包含该成员函数的类（例如 `ClassB`）必须在该友元声明之前已经被完整定义，或者至少已经被前向声明并且该友元函数是 `inline` 函数。如果该友元函数不是 `inline` 且 `ClassB` 只被前向声明，则需要先定义 `ClassB`，再定义 `ClassA`，最后再定义该友元函数。上面的例子中，由于 `ClassB` 的定义在 `ClassA` 之后，且 `displayAData` 不是 `inline`，所以需要先定义 `ClassB`，再定义 `ClassA`（并在其中声明友元），最后再实现 `displayAData`。或者更常见的是，`ClassB` 的定义在 `ClassA` 之前。
...
2.  非对称性 (Not Symmetric): 如果类 A 是类 B 的友元，这不意味着类 B 自动成为类 A 的友元。友元关系必须在需要访问的一方明确声明。
3.  非传递性 (Not Transitive): 如果 A 是 B 的友元，B 是 C 的友元，这不意味着 A 自动成为 C 的友元。友元关系不能传递。
4.  不可继承性 (Not Inherited): 友元关系不能被派生类继承。如果 Base 是 ClassX 的友元，那么从 Base 派生出的 Derived 类不是 ClassX 的友元。同样，如果 ClassX 是 Base 的友元，它不是从 Base 派生出的 Derived 类的友元。
...
然而，由于 `friend` 削弱了封装性并增加了类之间的耦合，应谨慎使用。只有当没有更优雅或高效的方式来实现所需功能时，才考虑使用 `friend` 。滥用 `friend` 可能导致代码更难理解、维护和修改。
...
所以，问题不是在于你是否写了 `= default` ，而在于编译器在看到 `= default` 后，能否成功合成一个可用的默认移动构造函数。它遵循与编译器隐式声明移动构造函数并为其生成默认实现相同的规则，但有一个关键区别： `= default` 请求编译器尝试合成，即使某些用户声明的特殊成员函数（如析构函数、拷贝构造函数）会阻止隐式生成。

总结来说：当你将移动构造函数设为 `= default` 时：

它会被定义（即编译器会合成一个可用的默认移动构造函数）的情况：

1.  当你没有用户声明任何会阻止默认移动操作生成的特殊成员函数（在 C++11/14 中是用户声明了析构函数、拷贝构造函数或拷贝赋值运算符会阻止隐式移动操作；在 C++17 及以后，这些用户声明不再阻止隐式移动操作，但仍然会影响是否需要 `= default` 来明确意图）。
2.  更重要的是，即使有用户声明的其他特殊成员函数，只要类中的所有非静态数据成员和直接基类都具有可访问且非 deleted 的移动构造函数（或具有可访问且非 deleted 的拷贝构造函数作为备用），并且它们不是 `const` 或引用类型。
...
1.  存在无法移动的非静态数据成员或直接基类：
    ...
    -   类包含一个非静态数据成员或直接基类，它具有用户声明的拷贝构造函数，但没有移动构造函数（在这种情况下，默认的移动构造会尝试拷贝它，如果拷贝也不行，或者拷贝与移动的语义不符，可能会导致问题或删除）。
2.  存在用户声明的移动构造函数或移动赋值运算符： 如果你已经用户声明了移动构造函数或移动赋值运算符，再 `= default` 通常是多余的，但不会导致它被 deleted。然而，如果你只用户声明了移动赋值运算符而没有移动构造函数，这并不会导致默认移动构造函数被删除，但可能意味着类的设计并不完全支持移动构造。
...
* 访问权限在类内部的放松： 当你在类内部定义一个函数时，这个函数对类中所有其他成员（包括公有、保护和私有成员）都拥有完全的访问权限。C++ 的访问控制（`public`, `protected`, `private`）主要限制的是类外部对成员的访问。
...
在 C++ 中，可以将聚合类直接通过 `emplace_back` 放入 `vector` ，但具体能否直接使用构造参数取决于 C++ 标准版本和初始化方式。以下是详细解释：
...
在C++中，以下两行代码的区别与初始化行为相关：

```cpp
std::array<int, 3> a;       // (1) 默认初始化
std::array<int, 3> a{};     // (2) 值初始化（通过空的花括号）
```

### 关键区别：

1. 默认初始化 (`std::array<int, 3> a;`):
   - `std::array` 是聚合类型（没有用户自定义构造函数）。
   - 对于内置类型（如 `int` ），默认初始化会保留元素的值未初始化（元素的值为随机垃圾值，取决于内存状态）。
   - 例如： `a[0]` , `a[1]` , `a[2]` 的值是未定义的。

2. 值初始化 (`std::array<int, 3> a{};`):
   - 空的花括号 `{}` 会触发聚合类型的值初始化。
   - 对于内置类型（如 `int` ），值初始化会将所有元素初始化为零。
   - 例如： `a[0]` , `a[1]` , `a[2]` 都会被初始化为 `0` 。
...
int main() {
    std::array<int, 3> a;     // 元素未初始化
    std::array<int, 3> b{};   // 元素初始化为0
...
* 默认初始化（无 `{}`）：元素未初始化（对内置类型是危险的，可能导致未定义行为）。
* 值初始化（带 `{}`）：元素被初始化为零（安全的选择，避免未初始化风险）。

推荐始终使用 `std::array<int, 3> a{};` 以确保初始化。

## C++允许派生类override函数与基类函数访问权限不同

例如 `QObject::eventFilter`
...
在 C++ 类设计中，决定将一个功能实现为成员函数（Member Function）还是同一命名空间下的非成员辅助函数（Non-member Helper Function），是接口设计中的一个关键问题。
...
如果一个函数可以通过调用类的公开接口（public interface）来实现其功能，而不需要访问私有数据，那么它最好被定义为非成员函数（通常放在同一命名空间下）。
...
*   成员函数代表了类的核心能力（Core capabilities），即那些必须接触底层数据的操作。
*   非成员函数代表了类的便利接口（Convenience interfaces）或扩展功能，它们利用核心能力来完成任务。
...
1.  `constexpr` 变量 (C++17 之前 vs 之后)：
    ...
    -   全局 `constexpr` 变量： 全局的 `constexpr` 变量默认具有内部链接属性 (Internal Linkage)，就像加了 `static` 一样。这意味着每个包含该头文件的 .cpp 文件都会拥有该变量的一份独立拷贝。如果你希望它是全局唯一的（虽然对于常量通常不重要，但如果取地址就有区别），你需要显式加上 `inline` (C++17 起)。
...
*   链接属性： 在 C++ 中，`const` 变量（以及 `constexpr` 变量，因为它们隐含 `const`）在命名空间作用域下默认具有内部链接属性 (Internal Linkage)。这相当于你显式加了 `static`。
*   结果： 如果 `a.cpp` 和 `b.cpp` 都包含这个头文件，编译器会为 `a.cpp` 生成一个 `MAX_VALUE`，为 `b.cpp` 也生成一个独立的 `MAX_VALUE`。
*   有问题吗？
    -   编译/链接： 没问题。因为它们是互不干扰的独立变量，链接器不会报“重复定义”。
    -   内存占用： 会有多份拷贝。如果你定义的是一个巨大的 `constexpr` 数组（例如查找表），每个包含该头文件的 .cpp 都会有一份该数组的副本，导致可执行文件变大。
    -   地址唯一性： `&MAX_VALUE` 在不同的 .cpp 文件中是不一样的。如果你的程序依赖于变量的地址（例如用于标识身份），这会出大问题。
...
*   C++17 及以后：
`static constexpr` 数据成员被隐式声明为 `inline` 。

    这意味着你不需要在 .cpp 文件中再写一遍 `constexpr int MyClass::Value;` 的定义。它在整个程序中只有一份定义，地址也是唯一的。
    结论： 不需要手动加 `inline` ，它已经是了。

*   C++17 之前：
    它不是 `inline` 的。虽然你可以直接用 `MyClass::Value` 进行计算，但如果你尝试对它取地址或者通过引用传递它，链接器可能会报错找不到符号，你需要在一个 .cpp 文件中提供一个不带初始化的定义。
...
一句话最佳实践：
在 C++17 及以上项目中，对于头文件里的全局常量，养成写 `inline constexpr` 的习惯是最好的（尤其是非基础类型）。对于函数和类静态成员，直接写 `constexpr` 即可。
...
在 C++ 编程中，在源文件中使用 `static` 函数（或者使用更现代的匿名命名空间）是一种良好的编程习惯，主要原因如下：
...
> *   逻辑链条：
    ...
    1.  如果你能定义出一个规则来限制数据的有效性（即有不变量），你应该使用 `class` 并通过构造函数和成员函数来强制执行这个规则。
    2.  如果你想不出什么规则限制数据（即没有不变量，任何值都是允许的），那么你处理的只是“纯数据”（plain data）。
...
当你在 C++ 中比较两个 `std::shared_ptr` （例如 `ptr1 == ptr2` ）时，编译器实际上比较的是它们内部存储的原始指针（Raw Pointer）的地址。

*   复制行为： 当你把一个 `shared_ptr` `push_back` 到一个容器（比如 `vector`）中时，实际上是发生了拷贝（copy）。容器里存的是一个新的 `shared_ptr` 对象，但是，这个新对象和原来的对象指向的是堆内存中完全相同的同一个对象。
...
这是一个非常深入且有趣的问题！它触及了 C++ 程序从源代码到可执行文件，再到加载运行的底层机制。
...
当你双击运行程序，或者在命令行输入 `./app` 时，操作系统的加载器 (Loader) 开始工作。
...
*   它只需要把那块已经在内存里的静态数据的地址（比如 `0x402010`），赋值给栈上的指针变量 `literal`。
...
所以，确实没人 `new` 它，它是程序自带干粮。

## C++中如何区分通用引用与右值引用

在 C++ 中，通用引用（Universal Reference）（现在官方术语称为 转发引用 Forwarding Reference）和右值引用（Rvalue Reference）在语法上看起来一模一样，都是 `T&&` 。
...
右值引用指向的是一个具体的类型，且不涉及类型推导。它只能绑定到右值（临时对象）。
...
在C++中，嵌套在有名命名空间（非匿名命名空间）内部的匿名命名空间，其访问性规则可以概括为：在当前翻译单元（Translation Unit）内，它等同于父级命名空间的直接成员；但在翻译单元外部，它是不可见的（具有内部链接性）。
...
*   不可见性： 匿名命名空间的主要目的是实现内部链接（Internal Linkage）。即使你在另一个 `.cpp` 文件中也定义了同名的 `namespace Outer`，你也无法访问到前一个文件中匿名命名空间里的成员。
...
*   `static` 只能修饰变量和函数。
*   匿名命名空间可以包含类定义、typedef、枚举等，并确保这些定义仅在当前翻译单元有效，不会污染全局符号表。
