...
![参数传递建议图一](assets/Pasted%20image%2020250723214928.png)
![参数传递建议图二](assets/Pasted%20image%2020250723214946.png)
...
这种模式会导致明显的死锁风险。某些种类的等待允许当前线程在异步操作完成前实施一些额外的工作。如果持有锁的线程实施了需要相同的锁的工作，那它就会因为试图获取它已经持有的锁而发生死锁。
...
R.12 告诉我们要避免原始所有权指针，与此相似， 我们也要尽可能避免原始线程和原始承诺（promise）。应使用诸如 std::async 之类的工厂函数， 它将处理线程的产生和重用，而不会将原始线程暴露给你自己的代码。
